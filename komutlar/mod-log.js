...
let logkanal = await db.fetch(`log_${message.guild.id}`)
  ...
  if (args[0] === "sıfırla" || args[0] === "kapat") {
   ...
   message.channel.send(`ModLog Kanalı Kapatıldı!`);
  
    return
  }
  
if (!logk) return message.channel.send(`Bir Kanal Belirtmen Lazım!`);

db.set(`log_${message.guild.id}`, logk.id)

message.channel.send(`ModLog Kanalı ${logk} Olarak Ayarlandı.`);
